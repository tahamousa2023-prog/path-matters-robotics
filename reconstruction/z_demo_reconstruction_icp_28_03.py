...
# ----------------------------------------------------------------------------
DEFAULT_SEED = 42
DEFAULT_CONFIDENCE_THRESHOLD = 10.0   # percentage 0-100, matching the official demo slider
                                       # 50 = keep the top 50% most confident points
                                       # raise to 70-80 for cleaner but sparser clouds
                                       # lower to 20-30 for denser but noisier clouds
DEFAULT_CONF_LOW_THRESHOLD = 10.0      # kept for CLI backwards compatibility, no longer used
...
# ----------------------------------------------------------------------------
DEFAULT_SCENE_DOWNSAMPLE_VOXEL = 0.00005   # 0.5mm voxel — was 1mm, halved to keep more detail
...
# ----------------------------------------------------------------------------
DEFAULT_OBJECT_DOWNSAMPLE_VOXEL = 0.0002   # was 0.01 (1cm) — 2mm keeps far more surface detail
...
DEFAULT_RANSAC_DOWNSAMPLE_VOXEL = 0.0003    # was 0.01 — 3mm keeps enough points for FPFH features
...
def parse_args():
    ...
    parser.add_argument("--yolo_crop", action="store_true",
                        help="Enable YOLO-based 2D segmentation → 3D projection crop. "
                             "When active, plane removal is skipped — YOLO handles background.")
...
class PipelineConfig:
    def __init__(self, args):
        ...
        # When YOLO crop is active, skip plane removal.
        # Plane removal before mask projection can cut object base points before
        # we even know what is object vs background.
        if args.yolo_crop:
            self.REMOVE_PLANE = False
            logging.debug("YOLO crop active → plane removal disabled")
...
def run_vggt_reconstruction(scene_dir: Path, conf_threshold: float, seed: int,
    ...
    # ------------------------------------------------------------------
    # Confidence filtering — percentage-based, matching the demo slider.
    #
    # predictions["world_points_conf"] is the per-point confidence from the
    # pointmap branch (range roughly 0–1 after sigmoid inside the model).
    # The demo slider is 0-100 and uses conf_thres directly as a percentile-
    # style threshold: keep points where conf >= conf_thres/100 * max_conf.
    #
    # We use world_points (pointmap branch) as primary source — the demo
    # uses this and it is generally higher quality than depth-unprojected
    # points for close-up objects.
    # ------------------------------------------------------------------
    world_points = predictions["world_points"]          # (S, H, W, 3) expected
    world_points_conf = predictions["world_points_conf"]  # (S, H, W) or (S, H, W, 1)
    ...
    images_np = predictions["images"]    # (S, 3, H, W) or (S, H, W, 3)
    ...
    # Limit to 200k points to keep downstream processing fast
    if len(points_3d) > 20000000:
        idx = np.random.choice(len(points_3d), 200000, replace=False)
    ...
    # ------------------------------------------------------------------
    # Build pixel coordinate grid for COLMAP (needed for track format).
    # We use depth-branch extrinsics/intrinsics since pointmap branch
    # produces the same camera geometry.
    # ------------------------------------------------------------------
    extrinsic_np = predictions["extrinsic"]    # (S, 4, 4)
    intrinsic_np = predictions["intrinsic"]    # (S, 3, 3)

    # Create a (S, H, W, 3) grid of [col, row, frame_idx] coords matching
    # world_points shape, then filter by the same mask
...
def run_pipeline(args):
    ...
    logging.info(f"YOLO crop: {'ENABLED' if config.YOLO_CROP else 'disabled'}")
    logging.info("="*70)
    ...
    # ========================================================================
    # STAGE 2b: YOLO OBJECT CROP  (optional, replaces SAM)
    # ========================================================================
    if config.YOLO_CROP:
    ...
    metrics = {
        'scale': float(scale),
        'yolo_crop_enabled': config.YOLO_CROP,
...
# """
# ### my usage:
# python /home/AP_PathMatters/vggt/z_demo_reconstruction_icp_28_03.py \
...
# # ----------------------------------------------------------------------------
# DEFAULT_SEED = 42
# DEFAULT_CONFIDENCE_THRESHOLD = 2       # primary (high) confidence threshold
# DEFAULT_CONF_LOW_THRESHOLD = 1.5         # secondary (medium) confidence threshold
#                                         # points with conf in [low, high) are kept only if
#                                         # they have enough high-conf neighbours (two-pass filter)
# DEFAULT_CONF_LOW_RADIUS = 0.05         # KD-tree search radius for neighbour check (scene units)
# DEFAULT_CONF_LOW_MIN_NEIGHBORS = 5     # min high-conf neighbours required to keep a low-conf point

# # ----------------------------------------------------------------------------
# # SCENE PREPROCESSING PARAMETERS
# # ----------------------------------------------------------------------------
# DEFAULT_SCENE_DOWNSAMPLE_VOXEL = 0.00001
# DEFAULT_SCENE_OUTLIER_NEIGHBORS = 20
...
#                                         # options: yolo11n-seg, yolo11s-seg, yolo11m-seg,
#                                         #          yolo11l-seg, yolo11x-seg  (n=nano … x=xlarge)
# DEFAULT_YOLO_NUM_IMAGES = 9            # how many images to run YOLO on (spread evenly)
# DEFAULT_YOLO_CONF = 0.25               # detection confidence threshold
# DEFAULT_YOLO_MIN_VOTES = 2             # keep 3D point if inside mask in >= N cameras
#                                         # 1 = most permissive, 2 = recommended (removes surface bleed)
#                                         # 3+ = very strict, may clip object edges
# DEFAULT_YOLO_CLUSTER_CLEANUP = True    # after vote filtering, run DBSCAN and keep only the
#                                         # largest cluster — removes residual surface fragments
# DEFAULT_YOLO_CENTER_WEIGHT = 0.6       # weight of center-proximity in detection scoring (0-1)
#                                         # 0.0 = pick purely by area (old behaviour)
...
# # OBJECT PREPROCESSING PARAMETERS
# # ----------------------------------------------------------------------------
# DEFAULT_OBJECT_DOWNSAMPLE_VOXEL = 0.001
# DEFAULT_OBJECT_NORMAL_RADIUS = 1.0
# DEFAULT_OBJECT_NORMAL_MAX_NN = 30
...
# # ----------------------------------------------------------------------------
# DEFAULT_RANSAC_TRIES = 20
# DEFAULT_RANSAC_DOWNSAMPLE_VOXEL = 0.001
# DEFAULT_RANSAC_MAX_CORRESPONDENCE_DISTANCE = 0.1
# DEFAULT_RANSAC_CORRESPONDENCE_CHECKER_DISTANCE = 0.1
# DEFAULT_RANSAC_NORMAL_RADIUS = 5.0
# DEFAULT_RANSAC_NORMAL_MAX_NN = 100
...
#     Uses two-pass confidence filtering:
#       - High confidence (>= conf_threshold): always kept.
#       - Medium confidence (>= conf_low, < conf_threshold): kept only if the
#         point has >= conf_low_min_neighbors high-conf neighbours within
#         conf_low_radius. Defaults to single-pass if conf_low is None or
#         equal to conf_threshold.
#     """
#     logging.info("\n" + "="*70)
#     logging.info("  VGGT RECONSTRUCTION")
#     logging.info("="*70)

#     # Default conf_low to conf_threshold → single-pass behaviour
#     if conf_low is None or conf_low >= conf_threshold:
#         conf_low = conf_threshold
...
#     # ------------------------------------------------------------------
#     # Two-pass confidence filtering
#     #
#     # Pass 1 — high confidence (>= conf_threshold): always keep
#     # Pass 2 — medium confidence (>= conf_low, < conf_threshold): keep
#     #           only if the point has >= conf_low_min_neighbors high-conf
#     #           neighbours within conf_low_radius.
#     #           Isolated low-conf points = noise.
#     #           Low-conf points surrounded by high-conf points = real geometry.
#     #
#     # If conf_low == conf_threshold this collapses to single-pass behaviour.
#     # ------------------------------------------------------------------
#     high_mask = depth_conf >= conf_threshold
#     high_mask_limited = randomly_limit_trues(high_mask.copy(), 100000)

#     pts_high = points_3d[high_mask_limited]
#     xyf_high = points_xyf[high_mask_limited]
#     rgb_high = points_rgb[high_mask_limited]
#     logging.info(f"Pass 1 (conf >= {conf_threshold}): {len(pts_high)} points")

#     all_pts = [pts_high]
#     all_xyf = [xyf_high]
#     all_rgb = [rgb_high]

#     if conf_low < conf_threshold and len(pts_high) > 0:
#         med_mask = (depth_conf >= conf_low) & (depth_conf < conf_threshold)
#         pts_med  = points_3d[med_mask]
#         xyf_med  = points_xyf[med_mask]
#         rgb_med  = points_rgb[med_mask]

#         logging.info(f"Pass 2 candidates (conf in [{conf_low}, {conf_threshold})): "
#                      f"{len(pts_med)} — running neighbour check "
#                      f"(radius={conf_low_radius}, min_nb={conf_low_min_neighbors})...")

#         if len(pts_med) > 0:
#             pcd_high_kd = o3d.geometry.PointCloud()
#             pcd_high_kd.points = o3d.utility.Vector3dVector(pts_high)
#             kd_tree = o3d.geometry.KDTreeFlann(pcd_high_kd)

#             kept_idx = [
#                 i for i, pt in enumerate(pts_med)
#                 if kd_tree.search_radius_vector_3d(pt, conf_low_radius)[0]
#                    >= conf_low_min_neighbors
#             ]

#             if kept_idx:
#                 idx = np.array(kept_idx)
#                 all_pts.append(pts_med[idx])
#                 all_xyf.append(xyf_med[idx])
#                 all_rgb.append(rgb_med[idx])
#                 logging.info(f"Pass 2 kept: {len(kept_idx)}/{len(pts_med)} "
#                              f"({100*len(kept_idx)/len(pts_med):.1f}%)")
#             else:
#                 logging.info("Pass 2: no medium-conf points passed neighbour check")

#     points_3d  = np.concatenate(all_pts, axis=0)
#     points_xyf = np.concatenate(all_xyf, axis=0)
#     points_rgb = np.concatenate(all_rgb, axis=0)
#     logging.info(f"Total after two-pass filter: {len(points_3d)} points")
...
#     # ------------------------------------------------------------------
#     # Save camera data for the YOLO crop stage.
#     #
#     # IMPORTANT: intrinsics must be saved at the ORIGINAL image resolution,
#     # not at the 518x518 processing resolution.
#     # rename_and_rescale_colmap already computed the rescaled params inside
#     # the COLMAP reconstruction object, so we read them back from there and
#     # convert to 3x3 K matrices.
#     #
#     # COLMAP PINHOLE params order: [fx, fy, cx, cy]
#     # ------------------------------------------------------------------
...
#         pyimage  = reconstruction.images[pyimageid]
#         pycamera = reconstruction.cameras[pyimage.camera_id]
#         cam_idx  = pyimageid - 1          # COLMAP IDs are 1-based
#         fx, fy, cx, cy = pycamera.params  # already rescaled to original res
#         K = np.array([[fx,  0, cx],
#                       [ 0, fy, cy],
#                       [ 0,  0,  1]], dtype=np.float64)
#         intrinsics_rescaled[cam_idx] = K

#     np.save(str(sparse_dir / "extrinsics.npy"), extrinsic)
#     np.save(str(sparse_dir / "intrinsics.npy"), intrinsics_rescaled)
#     logging.info(f"✓ Saved camera data: extrinsics.npy  intrinsics.npy "
#                  f"(rescaled to original image resolution)")
...
#     # ------------------------------------------------------------------
#     # STAGE B: Frustum / bbox crop
#     #   Discard any point outside the detection bbox in ALL cameras.
#     #   Fast coarse filter — removes most remaining background immediately.
#     # ------------------------------------------------------------------
#     logging.info(f"\n[Stage B] Frustum bbox crop ({len(bbox_data)} cameras)...")
#     # _frustum_bbox_crop expects (bbox, extr, intr, w, h) — strip cam_idx
#     bbox_data_for_frustum = [(b, e, i, w, h) for _, b, e, i, w, h in bbox_data]
#     bbox_keep = _frustum_bbox_crop(points_3d, bbox_data_for_frustum, expansion=0.10)
#     n_after_bbox = int(bbox_keep.sum())
#     logging.info(f"  {N_after_plane} → {n_after_bbox} points "
...
#     n_after_mask = int(combined_keep.sum())
#     logging.info(f"  {n_after_bbox} → {n_after_mask} points "
#                  f"({100*n_after_mask/N:.1f}% of original kept)")
