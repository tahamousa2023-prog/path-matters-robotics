...
DEFAULT_SCENE_NORMAL_MAX_NN = 30  # Max neighbors for normal estimation in scene
...
DEFAULT_RANSAC_FPFH_MAX_NN = 5  # Max neighbors for computing FPFH features
...
DEFAULT_SCALE_SEARCH_MIN_FACTOR = 0.5  # Minimum scale to test (0.5 = 50% of initial scale)
DEFAULT_SCALE_SEARCH_MAX_FACTOR = 1.5  # Maximum scale to test (1.5 = 150% of initial scale)
...
def compute_adaptive_parameters(object_pcd, scene_pcd, config):
    ...
    # ICP parameters - be more generous to allow matching
    config.ICP_MAX_CORRESPONDENCE_DISTANCE = reference_size * 0.05  # 5% of size
    
    # RANSAC parameters - need to be loose enough to find matches
    config.RANSAC_MAX_CORRESPONDENCE_DISTANCE = reference_size * 0.15  # 15% - more generous
    config.RANSAC_CORRESPONDENCE_CHECKER_DISTANCE = reference_size * 0.15
    config.RANSAC_TRIES = 50  # More attempts
    
    # FPFH/Normal radius - important for feature matching
    config.RANSAC_NORMAL_RADIUS = reference_size * 0.15  # 15% of size
    config.RANSAC_FPFH_RADIUS = reference_size * 0.15
    config.RANSAC_NORMAL_MAX_NN = 50
    config.RANSAC_FPFH_MAX_NN = 30  # More neighbors for better features
    
    # Downsampling - balance between detail and speed
    config.SCENE_DOWNSAMPLE_VOXEL = max(reference_size * 0.005, 0.001)  # 0.5% of size
    config.OBJECT_DOWNSAMPLE_VOXEL = max(obj_size * 0.01, 0.001)  # 1% of object
    config.RANSAC_DOWNSAMPLE = max(reference_size * 0.015, 0.005)  # 1.5% of size
...
def run_pipeline(args):
    ...
    logging.info(f"Object: {scene_dir / args.object_ply}")
    ...
    object_pcd = preprocess_object(
        pcd_path=Path(scene_dir / args.object_ply),
        config=config
    )
